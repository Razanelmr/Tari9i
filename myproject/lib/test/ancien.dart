...
class ReservationsPage extends StatelessWidget {
  ...
  Future<void> _cancelReservation(BuildContext context, String reservationId, Map<String, dynamic> reservation) async {
    try {
      ...
      final driverCommission = reservation['price'] != null ? (reservation['price'] * 0.1).round() : 0; // Assuming 10% commission
      final status = reservation['status'] ?? 'N/A';
      final date = reservation['date'] as Timestamp?;
      ...
      if (date != null && date.toDate().isBefore(DateTime.now().add(const Duration(hours: 24)))) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
      ...
      if (e.toString().contains('permission-denied')) {
        errorMessage = 'Vous n\'avez pas la permission d\'annuler cette réservation. Vérifiez votre connexion ou contactez le support.';
      } else {
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildInfoRow(
                                ...
                                "Date: ${date != null ? DateFormat('dd/MM/yyyy HH:mm').format(date) : 'Non spécifiée'}",
                              ),
                              _buildInfoRow(
                                Icons.event_seat,
